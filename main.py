...
def process_watershed(frame, mask):
    ...
    frame_copy[markers == -1] = [0, 0, 255]  # Viền bằng đỏ
    return frame_copy, unknown


# Load model YOLOv8 segmentation
model = YOLO(r"F:\University\Nam_ba\Do_an_TGMT\YOLOv8-seg\yolov8n-seg-seg\runs\segment\train\weights\best.pt")
...
# Khởi tạo biến thời gian cho FPS
prev_frame_time = time.time()
# Dùng stream=True để đọc trực tiếp webcam
for result in model.predict(source=1, stream=True, imgsz=640, conf=0.8, verbose=False):
    ...
    boxes = result.boxes
    names = result.names
    mask_img = np.zeros(frame.shape[:2], dtype=np.uint8)  # chỉ cần 1 kênh (grayscale)
    ...
    if masks is not None and boxes is not None:
        for i in range(len(masks.data)):
            ...
            watershed_img, thresh = process_watershed(frame_original, mask)  # ảnh kết quả watershed
            cv2.imshow("Thresh", thresh)

            # Cộng dồn mask nhị phân vào mask_img
            # mask_img = cv2.bitwise_or(mask_img, mask)
            # cropped = cv2.bitwise_and(frame, frame, mask=mask_img)
            # print(cropped.shape)
            if "rotten" in label:
                alpha = 0.5  # độ trong suốt khi overlay

                # Overlay watershed_img lên frame (frame đã có YOLO)
